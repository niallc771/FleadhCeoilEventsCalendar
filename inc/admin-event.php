<?php
	...
	$venue_info = get_option('venue_info');
...
<body>
	...
	<ul>
        ...
        <!-- added the code below to create the drop down list with the list of venues in them too -->
		<?php if ($aec_options['venue'] > 0) { ?>
		<li>
			
			<label for="venue"><?php _e('Venue', AEC_NAME); ?></label>
			<select name="venue_id" id="venue_id" onchange="fillAddress(this.value)"><option value="-1">---</option>
	<?php
            foreach($venue_info as $key => $value){
                    $selected = $event->venue_id == $value->venue_id ? " selected" : "";
                 echo "<option value='$value->venue_id'$selected>$value->venue_name</option>";
                }
			?>
			</select>
		</li>
...
<script>
// code for when clicking on a location in add event form it fills out other options
function urldecode(str) { // urldecode decodes URL-encoded string
   return decodeURIComponent((str+'').replace(/\+/g, '%20'));
}
var Venue = function(id, name, street, city, state, zip, website)
{
    this.id = id;
    this.name = name;
    this.street = street;
...
<?php
foreach($venue_info as $key => $value){
echo "venues[$value->venue_id] = new Venue($value->venue_id, '" . urlencode($value->name) . "', '" . urlencode($value->street) . "', '" . urlencode($value->city) . "', '" . urlencode($value->state) . "', '" . urlencode($value->zip) . "', '" . urlencode($value->website) . "')\n";
}
?>
function fillAddress(id)
{
    ...
    document.getElementById("address").value = urldecode(venues[id].street);
    if(venues[id].street != "") {
        document.getElementById("address").setAttribute("class", "wide");
        for(var label in document.getElementsByTagName("label"))
            if(document.getElementsByTagName("label")[label].getAttribute("for") == "address") {
                document.getElementsByTagName("label")[label].setAttribute("class", "");
                break;
            }
    }
    
    document.getElementById("city").value = urldecode(venues[id].city);
    if(venues[id].city != "") {
        document.getElementById("city").setAttribute("class", "auto");
        for(var label in document.getElementsByTagName("label"))
            if(document.getElementsByTagName("label")[label].getAttribute("for") == "city") {
                document.getElementsByTagName("label")[label].setAttribute("class", "");
                break;
            }
    }

    document.getElementById("state").value = urldecode(venues[id].state);
    if(venues[id].state != "") {
        document.getElementById("state").setAttribute("class", "auto");
        for(var label in document.getElementsByTagName("label"))
            if(document.getElementsByTagName("label")[label].getAttribute("for") == "state") {
                document.getElementsByTagName("label")[label].setAttribute("class", "");
                break;
            }
    }
    
    document.getElementById("zip").value = (venues[id].zip != "") ? urldecode(venues[id].zip) : "none";
    

    document.getElementById("zip").setAttribute("class", "auto");
    for(var label in document.getElementsByTagName("label"))
        if(document.getElementsByTagName("label")[label].getAttribute("for") == "zip") {
            document.getElementsByTagName("label")[label].setAttribute("class", "");
            break;
       
        }    
        
    document.getElementById("link").value = urldecode(venues[id].website);
}
</script>
</body>
</html>
