 ...
 */
var filter;
var selectedCategory; // used in calendar.php
var selectedCatName; // used in calendar.php

jQuery(document).ready(function ($) {
    filter = function (active) { // Called when a category button is clicked
        selectedCategory = active;  // Save the category button
            selectedCatName = "all";
            document.getElementById("venues").selectedIndex = 0; // Reset the venue dropdown to 'All Venues'
            var classList = $(selectedCategory).attr('class').split(/\s+/);
            $.each(classList, function (index, item) {
                if (item.startsWith('cat')) {
                    selectedCatName = item; // Save the category name e.g. 'cat5' so when filtering by venue it only filters the current category
                }
            });
            
        	/*$('#aec-filter li').click(function(){ 
        	   if($(this).hasClass('active')) { 
        	       $(this).fadeTo(0, 0.25).removeClass('active');  
                } 
                else { 
                    $(this).fadeTo(250, 1).addClass('active');
                }           
                calendar.fullCalendar('rerenderEvents');
            });*/ 
            
			$('#aec-filter li').next().fadeTo(0, .5).removeClass('active'); // fades the colour of the category buttons
		...
		calendarInit = function () {
			...
			calendar	= $('#aec-calendar').fullCalendar({
				...
				header: {
					...
					center: 'title',
					right: 'month,agendaWeek,agendaDay' //Romo renders day button in admin side!!!
				},
