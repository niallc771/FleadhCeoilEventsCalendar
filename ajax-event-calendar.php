...
define('AEC_LOCATION_TABLE', 'aec_location');
...
if (!class_exists('ajax_event_calendar')) {
	class ajax_event_calendar {
		...
		function __construct() {
		    $this->calendar_name = "Fleadh Cheoil Event's Calendar"; // the name of the calendar
		...
		function install() {
			require_once(ABSPATH . 'wp-admin/includes/upgrade.php');// creates the event table below, deleted the oringal venue string with venue_id now
			global $wpdb;
		...
		if (version_compare($current, '1.0.1', '<')) {
			...
			if ($plugin_updated) {
				...
				$_POST['event']['title'] = "Fleadh Cheoil Events Calendar [v" . AEC_VERSION . "] successfully installed!";
	    ...
	    function localize_plugin($page) {
			...
			if (get_option('timezone_string')) {
				...
				$this->set_admin_alert(sprintf('<p>Fleadh Cheoil Events Calendar %s.<br>%s... <a href="http://www.travelmath.com/time-zone/" target="_blank"><strong>%s</strong></a>.</p>
			...
			if (get_magic_quotes_gpc()) {
				$this->set_admin_alert(sprintf('<p>%s %s <br>%s <a href="http://wordpress.org/support/topic/plugin/fleadh_cheoil_events_calendar/ajax-event-calendar-dont-like-the-apostrophes?replies=11#post-2259386" target="_blank"> <strong>%s</strong></a>. %s</p>'
				, __('Your server has PHP magic_quotes_gpc set to active.', AEC_NAME)
				, __('This produces formatting errors in the Fleadh Cheoil Events Calendar plugin.', AEC_NAME)
		...
		function render_admin_menu() {
			...
			if (function_exists('add_options_page')) {
				...
				$page = add_menu_page('Fleadh Cheoil Events Calendar',  __('Calendar', AEC_NAME), 'aec_add_events', AEC_FILE, array($this, 'render_admin_calendar'), AEC_URL . 'css/images/em-icon-16.png', AEC_MENU_POSITION);
		...
		function render_shortcode_calendar($atts) {
			extract(shortcode_atts(array(
				...
				'year'			=> date('Y'),
				'view' 			=> 'month',
				'views' 		=> 'month,agendaWeek,agendaDay', // adding the extra day view here(agendaDay)
				'nav'			=> 'prev,next',
				'height'		=> '',
				'scroll'		=> 0,
				'mini'			=> 0
...
/* Venue Drop*/ $out .= '<li>';
/* NC        */ $out .= '<select name="venue_id" style="width:160px; margin-right:25px; margin-left:15px" id="venues" onchange="filterVenues(this.value)"><option value="-1">All Venues</option>';
                foreach($venues as $v) {
                    $out .= '<option value="venue'.$v->venue_id.'">'.$v->venue_name.'</option>';
                }
                $out .= '</select>';
				$out .= '</li>';
		...
		function render_admin_venue() {
			...
			$add .= "<form action='http://localhost/wordpress/wp-content/plugins/fleadh_cheoil_events_calendar/location-output.php' method='post' id='aec-venue-form'>\n";
			// Venue
        ...
        function render_admin_export() { 
            ...
            $add = $this->add_wrap(__('<h1>Export Venues</h1>', AEC_NAME)); 
            $add .= "<form action='http://localhost/wordpress/wp-content/plugins/fleadh-cheoil-events-calendar/csv/export/export.php' method='post' id='aec-export-form'>\n"; 
        ...
        function render_admin_export_events() { 
            ...
            $add .= "<form action='http://localhost/wordpress/wp-content/plugins/fleadh-cheoil-events-calendar/csv/export/exportevents.php' method='post' id='aec-export-form'>\n"; 
            $add .= $this->add_wrap(__('Create CSV File', AEC_NAME), "<br><br><button type='submit' class='add button-primary'>", "</button></p>"); 
            $add .= "</form>\n"; 
            echo($add); 
        }
		...
		function generate_event($input, $user_id) {
			...
			$output 		= array(
				...
				'overlap_list'	=> $input->overlap_list,
		...
		function delete_venue() {
			...
			$this->db_delete_venue($_POST['venue_id']);
		}
		...
		function db_query_events($start, $end, $category_id, $excluded, $limit=false) {
			...
			$sql = ("SELECT id, venue_id, user_id, title, start, end, allDay, repeat_int, repeat_freq, repeat_end, category_id , 
            IFNULL((SELECT group_concat(e2.title SEPARATOR '&rdquo;, &ldquo;') FROM wp_aec_event e2 
            WHERE e2.venue_id = e1.venue_id 
            AND ((e2.start > e1.start AND e2.start < e1.end) OR (e1.start > e2.start AND e1.start < e2.end))), '0') as overlap_list , 
            (SELECT count(e2.id) FROM wp_aec_event e2 WHERE e2.venue_id = e1.venue_id AND ((e2.start > e1.start AND e2.start < e1.end) 
            OR (e1.start > e2.start AND e1.start < e2.end))) as overlap_count 
										FROM " . $wpdb->prefix . AEC_EVENT_TABLE . " e1
										WHERE (
        ...
        function db_get_venues()
        {
        	global $wpdb;
			$result = $wpdb->get_results('SELECT venue_id, venue_name FROM ' . $wpdb->prefix . AEC_VENUE_TABLE .  ' ORDER BY venue_id;');
			return $this->return_result($result);
        }
		...
		function generate_css() {
			...
			$out .= '#aec-filter li{float:left;}';
            $out .= '#start_date_cal{position:relative; top:-2px; font-size:15px; font-weight:bold; border-radius:5px}';
			$out .= "</style>";
			return $out;
		}
		...
		function generate_css_venue() {
			...
			foreach ($venues as $venue) {
				$out .= ".ven{$venue->venue_id}";
				$out .= ",.ven{$venue->venue_id} .ven-skin";
			}
			$out .= "</style>";
			return $out;
		}
